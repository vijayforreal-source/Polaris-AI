...
export default function AntarcticMap({ metadata, grid, icebergs }) {
  ...
  useEffect(() => {
    ...
    const map = new Map({ target: mapTarget.current, layers: [landLayer, createSeaIceLayer(grid), icebergLayer, createBharatiLayer(metadata.bharati)], view: new View({ projection: ANTARCTIC_CRS }), controls: [] });
    map.on("singleclick", (event) => {
      const feature = map.forEachFeatureAtPixel(
        event.pixel,
        (candidate) => candidate.get("iceberg") ? candidate : null,
        { hitTolerance: 8 },
      );
